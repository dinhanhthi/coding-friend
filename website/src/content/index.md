...
- `/cf-plan` ([source](https://github.com/dinhanhthi/coding-friend/blob/main/plugin/skills/cf-plan/SKILL.md)) — Brainstorms with you, then writes a phased plan under `docs/plans/<plan-name>/` (plus `brief.md` with the original request and Q&A in normal/hard mode). Under the hood: `cf-explorer` maps the repo, `cf-planner` compares approaches, then `cf-implementer` runs each task. Available modes:
  ...
  - `--commit-per-task` — with autopilot, commit each task as `phase N/M task i/K <title>` and review once per phase over those commits; `[parallel]` phases run sequentially. Also on when config `planCommitPerTask: true`
  - `--no-commit-per-task` — force commit-per-task off for this run, even if `planCommitPerTask: true`
...
- `/cf-plan-resume` ([source](https://github.com/dinhanhthi/coding-friend/blob/main/plugin/skills/cf-plan-resume/SKILL.md)) — Reloads a saved plan and its context file, skips DONE tasks, re-runs the rest via the same execute protocol. If the plan has `auto: true` and an `AUTOPILOT` section, it continues in autopilot, honoring frontmatter `commitPerTask: true`; `--commit-per-task` / `--no-commit-per-task` override it.
- `/cf-plan-review` ([source](https://github.com/dinhanhthi/coding-friend/blob/main/plugin/skills/cf-plan-review/SKILL.md)) — Reviews a saved plan folder with a fresh in-session reviewer before you implement it; `--codex` / `--gemini` / `--claude` / `--cursor` / `--grok` add external reviewers in parallel and merge. `--fix` (`--inline`) applies Critical/Important findings immediately. Writes `review.md` next to the plan and, without `--fix`, offers to apply Critical/Important findings.
...
| `review`                | Review settings. Nested object; `withCodex` runs a Codex second opinion; `withClaude` / `withGrok` / `withCursor` / `withGemini` add that headless external reviewer (all default `false`); `agentTimeout` (default 300) is enforced on each external reviewer subprocess; `nativeTimeout` (default 600) is a cooperative budget for each in-session reviewer job; `maxRounds` (default 5) caps the autopilot review-fix loop and the `/cf-review --fix` loop. |
...
| `planCommitPerTask`     | Boolean. Under `/cf-plan` autopilot, commit each task separately and review once per phase (range review, so `review.with*` external reviewers do not run). Ignored without autopilot. `--no-commit-per-task` forces off. `/cf-plan-resume` ignores this key but honors frontmatter `commitPerTask: true` and accepts `--commit-per-task` / `--no-commit-per-task`. Default: `false`.                                                                          |
