...
6. **Install deps & import** — installs SQLite deps and imports existing memories
7. **MCP setup** — adds `coding-friend-memory` to `.mcp.json`
...
The daemon runs detached, auto-rebuilds on file changes, and auto-stops after idle timeout (default: 30 min). If it stops, the MCP server respawns it automatically on the next request.
