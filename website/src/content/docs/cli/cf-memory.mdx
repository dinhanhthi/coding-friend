...
| `cf memory mcp`             | Print MCP server config (for non-Claude-Code clients) and run health checks |
...
6. **MCP setup** — registers `coding-friend-memory` at **user scope** via `claude mcp add --scope user` (no per-project `.mcp.json` needed)
