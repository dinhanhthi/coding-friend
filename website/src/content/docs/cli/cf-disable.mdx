...
keywords:
  [disable, deactivate, turn off, cf disable, scope, codex, agy, antigravity]
---
...
cf disable --agent codex # Disable Coding Friend in Codex config
cf disable --agent agy   # Disable Coding Friend in Antigravity config
cf disable --agy         # Alias for --agent agy
```

For Codex CLI, `cf disable --agent codex` records `coding-friend` as disabled in `~/.codex/config.toml`.

For Google Antigravity, `cf disable --agent agy` sets `plugins["coding-friend"].enabled` to `false` in `~/.gemini/config/config.json` without deleting plugin files. Restart Antigravity, then `cf enable --agy` to turn it back on.
