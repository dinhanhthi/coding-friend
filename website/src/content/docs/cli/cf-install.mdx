---
title: "cf install"
description: "Install the Coding Friend plugin into Claude Code, Codex CLI, or Google Antigravity from the terminal."
keywords:
  [
    install,
    setup,
...
`cf install` installs the Coding Friend plugin for the selected host. Claude Code is the default; Codex CLI is opt-in with `--agent codex` or `--codex`; Google Antigravity is opt-in with `--agent agy` or `--agy`.
...
cf install --agent agy   # Copy the Antigravity plugin + enable it (beta)
cf install --agy         # Alias for --agent agy
```

## Scopes

Scope flags apply to Claude Code installs. Codex CLI uses Codex's own global plugin marketplace plus project setup from [`cf init --agent codex`](/docs/cli/cf-init/). Google Antigravity always installs to `~/.gemini/config/plugins/coding-friend/` — there is no user/project/local scope.
...
1. **Checks prerequisites** — verifies that `agy` exists and is at least `1.1.0`
2. **Copies the plugin** — deploys `plugin-antigravity/` to `~/.gemini/config/plugins/coding-friend/`
3. **Enables the plugin** — writes `plugins["coding-friend"].enabled: true` in `~/.gemini/config/config.json`
4. **Registers memory MCP** — writes `coding-friend-memory` into the plugin's `mcp_config.json`
5. **Validates** — runs `agy plugin validate` on the installed plugin (warnings do not roll back the copy)
...
For Google Antigravity, `cf install` copies files (no marketplace CLI):

```bash
# plugin lands at ~/.gemini/config/plugins/coding-friend/
...
- For Google Antigravity, run `cf init --agent agy`, then restart Antigravity.
