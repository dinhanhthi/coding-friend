...
- **Tier 2 (Plugin Scripts)** — Two wide rules that cover all Coding Friend plugin scripts, handling both quoted and unquoted path invocations. These patterns are version-independent, so permissions survive plugin updates automatically — no refresh needed.
