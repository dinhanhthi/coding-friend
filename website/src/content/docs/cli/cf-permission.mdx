---
title: "cf permission"
description: "Manage Coding Friend permissions for Claude Code, Codex CLI, and Google Antigravity."
keywords:
  [
    permission,
    permissions,
    claude code,
    codex cli,
    antigravity,
    agy,
    settings,
    allow rules,
    cf permission,
  ]
---

## Overview

Interactive permission manager for Coding Friend. By default it configures Claude Code permission rules so skills and hooks can run without repeated prompts. Use `--agent codex` for the Codex-specific auto-approve toggle, or `--agent agy` for Antigravity (native `/permissions` plus the shared `autoApprove` config key).

```bash
cf permission
```

For the full list of rules and which skills use them, see [Permissions Reference](/docs/reference/permissions/).

💡 You can run it multiple times to add or remove rules.

## Usage

```bash
# Interactive — browse categories and pick rules
cf permission

# Apply all recommended permissions without prompts
cf permission --all

# Choose scope explicitly
cf permission --project    # .claude/settings.local.json (default)
cf permission --user       # ~/.claude/settings.json (all projects)

# Combine: apply all to user scope
cf permission --all --user

# Codex deterministic auto-approve
cf permission --agent codex --enable-auto-approve
cf permission --agent codex --disable-auto-approve

# Antigravity — native permissions; auto-approve is a config key
cf permission --agent agy
cf permission --agy
```

## How It Works

1. Asks where to save permissions (project or user scope)
2. Shows permission categories with progress indicators (e.g., "3/5 ✓") plus an **Auto-approve** toggle
3. Lets you drill into each category to check/uncheck individual rules — each rule has a colored tag prefix indicating its type (`[read-only]`, `[modify]`, `[write]`, `[remote]`, `[execute]`, `[network]`)
4. The Auto-approve toggle lets you enable/disable auto-approve (auto-approves read-only tools + working-dir file edits, LLM classifier for unknowns) and choose whether to save the setting globally or per-project. When enabling, it audits existing permissions for dangerous rules that would bypass the classifier
5. Computes the diff (rules to add, rules to remove)
6. Asks for confirmation before writing changes
7. Automatically cleans up stale old-format per-script rules

## Scope

By default, `cf permission` asks you to choose between:

- **Project** (`.claude/settings.local.json`) — scoped to this project only, gitignored
- **User** (`~/.claude/settings.json`) — applies to all projects

Use `--project` or `--user` to skip the prompt.

## Codex CLI

Codex permissions are separate from Claude Code permission rules. `cf permission --agent codex --enable-auto-approve` writes `autoApprove: true` to `.coding-friend/config.json` — the same key shared with Claude and Antigravity.

Codex auto-approve is deterministic-only: known-safe reads and working-directory edits can be approved, destructive actions are blocked, and unknown actions defer to Codex native approval. The Claude Sonnet classifier is not used from Codex hooks.

## Google Antigravity

Antigravity permissions are managed natively (`/permissions` in `agy`). `cf permission --agent agy` does not write allow rules; it reminds you that Coding Friend auto-approve is the same `autoApprove` key as Claude in `.coding-friend/config.json`:

```json
{
  "autoApprove": true
}
```

Antigravity auto-approve is deterministic-only: known-safe reads and working-directory edits can be allowed, destructive actions are denied, and unknown actions return `ask`. The Claude Sonnet classifier is not used from Antigravity hooks. There is no `--enable-auto-approve` flag for `--agent agy`.

## Permission Tiers

Coding Friend organizes permissions into two tiers:

- **Tier 1 (Static)** — Path-independent rules like `Bash(git add *)`, `Bash(npm test *)`, MCP memory tools. These are stable across plugin updates.
- **Tier 2 (Plugin Scripts)** — Four rules that cover all Coding Friend plugin scripts: two Bash rules (quoted and unquoted path invocations) and two Read rules (plugin cache and global config). These patterns are version-independent, so permissions survive plugin updates automatically — no refresh needed.

When adding Plugin Script permissions, `cf permission` shows a warning explaining the wide pattern and asks for confirmation.

## Also Available In

- **`cf init`** — Step 8 offers to apply all recommended permissions during initial setup
- **`cf config`** → Permissions — same options accessible from the config menu

## See Also

- [Permissions Reference](/docs/reference/permissions/) — full permission list with descriptions
- [`cf init`](/docs/cli/cf-init/) — initial setup wizard (Step 8 handles permissions)
- [`cf config`](/docs/cli/cf-config/) — edit settings including permissions
- [Security](/docs/reference/security/) — security model
