...
- **Tier 2 (Plugin Scripts)** — A single wide rule that covers all Coding Friend plugin scripts (e.g., `Bash(bash ~/.claude/plugins/cache/.../coding-friend *)`). This pattern is version-independent, so permissions survive plugin updates automatically — no refresh needed.
