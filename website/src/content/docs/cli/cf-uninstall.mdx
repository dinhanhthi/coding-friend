---
title: "cf uninstall"
description: "Remove or disable the Coding Friend plugin from Claude Code, Codex CLI, or Google Antigravity."
keywords:
  [uninstall, remove, cleanup, cf uninstall, scope, codex, agy, antigravity]
---
...
Use `cf uninstall --agent codex` for Codex CLI, or `cf uninstall --agent agy` for Google Antigravity.
...
### Google Antigravity (`--agent agy`)

Removes the copied plugin at `~/.gemini/config/plugins/coding-friend/`, unregisters memory MCP from `mcp_config.json`, and deletes the `plugins["coding-friend"]` entry in `~/.gemini/config/config.json`. Restart Antigravity afterwards.
