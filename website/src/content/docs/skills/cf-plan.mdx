...
After your plan is ready and implementation is complete, follow this order:

```
/cf-plan → implement → /cf-review → /cf-commit
```

The plan automatically reminds you to run `/cf-review` before committing, so you don't skip the review step and jump straight to `/cf-commit`.
