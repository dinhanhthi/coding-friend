...
/cf-plan --model opus [task description]  # Pin cf-planner model at brainstorm (alias also --model=opus)
```

<Callout type="info" title="Resuming a plan">
  Resuming an interrupted plan moved to its own command: [`/cf-plan-resume <plan>`](/docs/skills/cf-plan-resume/). This keeps `/cf-plan` lean and avoids loading the full planning workflow just to continue execution.
</Callout>
...
- **Model** (`--model <alias>`) — Orthogonal, planner-only: pin the model for **cf-planner** at brainstorm. Does not affect cf-explorer, cf-implementer, or cf-plan itself. Skipped when fast mode is active (including auto-detect); still works with `--hard`. Valid aliases: `opus` / `sonnet` / `haiku` / `fable`. Invalid values warn and continue.
...
# Pin cf-planner to opus for brainstorming
/cf-plan --model opus Add a healthz endpoint
```
...
Optionally, `/cf-plan` can generate a concise, human-readable overview doc alongside the agent plan. It is decision-focused (what was chosen and why), includes an ASCII diagram of the phases, and is a point-in-time snapshot — not updated as tasks complete. It is **off by default** because it costs extra tokens.
