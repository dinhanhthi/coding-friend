...
- Save the plan to `docs/plans/` — single file for small plans, subfolder with chunks for large ones (8+ tasks or 3+ phases)
...
Plans are saved to `docs/plans/`. The structure depends on plan size:

**Small plan** (< 8 tasks and < 3 phases) — single file:
...
**Big plan** (8+ tasks or 3+ phases) — subfolder with chunks:
