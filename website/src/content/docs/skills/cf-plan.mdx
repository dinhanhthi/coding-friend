...
- Save the plan to `docs/plans/` (unless `--fast` with a single phase — those stay in chat); single file for single-phase plans, subfolder with chunks for multi-phase plans
...
Plans are saved to `docs/plans/` (except `--fast` single-phase plans, which stay in chat). The structure depends on phase count:

**Single-phase plan** — single file:

```
docs/plans/
  ├── 2026-03-05-add-cli-command.md
  └── ...
```

**Multi-phase plan** (2+ phases) — subfolder with chunks:
