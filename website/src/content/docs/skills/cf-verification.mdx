...
keywords: [verification, completion gate, quality check]
---
...
Automatically triggered after tasks that **produce or modify code**:
...
**Does NOT auto-invoke** after analysis-only skills that don't change code:
...
This avoids duplicate test runs in workflows like `/cf-plan → [implement] → /cf-review → /cf-commit`. Tests run once after implementation via `cf-verification`, then `/cf-review` and `/cf-commit` skip re-running them.
...
Scan changed code for signs of AI over-generation:

- **AI transmarks** — phrases like "Certainly!", "Of course!", "I'd be happy to" in production code
- **Excessive comments** — comments restating obvious code or function names
- **Unnecessary verbosity** — over-engineered abstractions, verbose error handling wrapping simple operations
