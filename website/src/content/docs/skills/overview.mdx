...
You can extend any built-in skill with your own guidance by creating a `.md` file in `.coding-friend/skills/` (per-project) or `~/.coding-friend/skills/` (global). Each file supports 3 optional sections:
...
For example, `.coding-friend/skills/cf-commit.md` can add custom commit rules or trigger tests after each commit. See [Configuration](/docs/configuration/config-json#custom-skill-guides) for full details.
