...
Documents include YAML frontmatter (title, description, tags) for efficient future recall by `/cf-fix` and `cf-sys-debug` itself. After saving, confirms with a 2-line summary: the markdown file path and MCP database indexing status.
