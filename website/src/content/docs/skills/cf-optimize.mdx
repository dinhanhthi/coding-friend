...
6. **Implement** — Dispatches the [`cf-implementer`](/docs/reference/agents/#cf-implementer) agent: verify existing tests pass, implement the optimization, confirm no regressions. Pass `--add-tests` to enable TDD (write benchmark/regression tests first)
...
9. **Auto-Review** — Automatically runs [`/cf-review`](/docs/skills/cf-review/) after the optimization is verified
...
- **TDD Integration** — Opt-in test-first via `--add-tests` flag with the [`cf-implementer`](/docs/reference/agents/#cf-implementer) agent
