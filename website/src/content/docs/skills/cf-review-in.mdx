...
7. **Offer fixes** — If confirmed or questionable issues found, offer to fix them via cf-implementer (dismissed findings are never auto-fixed). Pass `--add-tests` to enable TDD for the fixes
