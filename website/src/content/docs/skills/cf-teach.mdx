...
<Callout type="tip" title="CLI Requirement: NONE">
  Works without `coding-friend-cli`. See [CLI requirements](/docs/reference/cli-requirements/) for the full matrix.
</Callout>
...
4. **Saves to Output** — Writes the narrative doc to `~/.coding-friend/learn/` (same directory as `/cf-learn`)
...
```
~/.coding-friend/learn/debugging/how-i-debugged-the-race-condition.md
~/.coding-friend/learn/concepts/why-we-chose-redis-over-postgres.md
~/.coding-friend/learn/patterns/the-refactor-that-untangled-the-api-layer.md
```
...
Both skills write to the same configurable directory (default: `~/.coding-friend/learn/`), but they serve different purposes:
...
| `learn.outputDir`  | `~/.coding-friend/learn` | Where to store teaching docs (always global, never project-local)          |
| `learn.autoCommit` | `false`                  | Auto git-commit after writing                                              |

Configure in `~/.coding-friend/config.json` (global config only — learn is always global).
