...
💡 To see what you've learned so far, run [`cf learn host`](/docs/cli/cf-learn/) to view the learning notes in your browser or use [`cf mcp`](/docs/cli/cf-mcp/) to integrate with other LLM clients.

To serve your learning docs via MCP, run `cf mcp` — it auto-registers the Learn MCP server at user scope (available across all projects). See [MCP Servers → Learn MCP](/docs/reference/mcp/#learn-mcp) for setup details and available tools.
...
5. **Saves to Output** — Stores in configurable directory (default: `~/.coding-friend/learn/`)
...
All settings are optional. Configure in `~/.coding-friend/config.json` (global config only — learn is always global).

| Setting             | Default                  | Description                                                                                         |
| ------------------- | ------------------------ | --------------------------------------------------------------------------------------------------- |
| `learn.language`    | `en`                     | Language for learning notes (falls back to top-level `language`, then `en`)                         |
| `learn.outputDir`   | `~/.coding-friend/learn` | Where to store learning docs (always global, never project-local)                                   |
...
Both skills write to the same configurable directory (default: `~/.coding-friend/learn/`), but serve different purposes:
