...
The optional topic argument acts as a **filter** for the conversation:

- **With argument**: Focuses only on conversation content related to the topic — e.g., `/cf-learn async error handling` extracts only async/error-related knowledge
- **Without argument**: Scans the full conversation for the most valuable learnings

The topic name for the output file is always **auto-detected** from the content, not taken verbatim from your argument. Your argument narrows _what_ to extract, not _what to name the file_.
