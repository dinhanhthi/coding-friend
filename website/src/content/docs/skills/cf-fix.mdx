...
6. **Implement Fix** — Dispatches the [`cf-implementer`](/docs/reference/agents/#cf-implementer) agent to fix the root cause. If `--add-tests` was passed, writes a failing regression test first (TDD); otherwise fixes directly using existing tests
...
8. **Save Bug Knowledge** — If the fix required more than 1 attempt, documents the bug (symptom, root cause, fix, prevention) to `docs/memory/bugs/` via [`cf-writer`](/docs/agents/cf-writer/) agent; confirms with a 2-line summary: the markdown file path and MCP database indexing status
...
- **TDD Integration** — Opt-in test-first for bug fixes via `--add-tests` flag
