...
- **All changed files are non-code** — styles (`.css`, `.scss`, `.sass`, `.less`, `.styl`), docs (`.md`, `.mdx`, `.txt`, `.rst`), or config (`.json`, `.yaml`, `.yml`, `.toml`, `.env`, `.ini`, `.lock`, `.config`)
- **User passes `--no-tdd`** — explicitly opts out with a one-line acknowledgment
