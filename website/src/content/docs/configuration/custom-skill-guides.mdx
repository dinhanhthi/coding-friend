...
Every `cf-*` skill has a **Step 0** that auto-runs a bash script (`load-custom-guide.sh`) via an inline shell-injection block:

````markdown
```!
bash "${CLAUDE_PLUGIN_ROOT}/lib/load-custom-guide.sh" cf-<skill>
```
````

Because the block is a `!`-prefixed injection, Claude Code executes it **at invocation, before the model reads the skill** — the command output replaces the block in place. This guarantees the guide is present in context and cannot be skipped. The script checks the local path first, then the global path; if a guide is found, its output is injected and the skill integrates the sections into its workflow:
...
Custom guides are loaded **on-demand** — only when the corresponding skill runs. Changes to a guide take effect on the next skill invocation (no `/clear` needed). Shell injection can be turned off globally with `"disableSkillShellExecution": true` in settings; if disabled, the raw command stays visible and the skill runs it manually as a fallback.
