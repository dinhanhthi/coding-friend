...
- **Type**: `boolean`
- **Default**: `false`
- Enables deterministic-only auto-approve for Google Antigravity
- Unknown actions return `ask`; the Claude Sonnet classifier is not used from Antigravity hooks
- Set in `.coding-friend/config.json`. `cf permission --agent agy` documents the key; it does not toggle it
