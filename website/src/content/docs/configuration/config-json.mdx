...
- **Type**: `boolean`
- **Default**: `false`
- When `true`, [/cf-plan](/docs/skills/cf-plan/) skips generating the human-readable overview doc alongside the agent plan
- Per-run override: pass `--no-gui` (alias `--no-human`) to disable the overview for a single run without changing config
...
- When `true`, every `/cf-review` (including the auto-invoked ones from `/cf-plan`, `/cf-fix`, `/cf-optimize`) runs `codex review --uncommitted` in parallel and merges both reviews into one report
