...
- **Codex CLI:** deterministic rules only; unknown actions defer to Codex native approval; no Claude Sonnet classifier
- One key enables **all** hosts. There is no per-host override
- Set with `cf init`, `cf init --agent agy`, `cf init --agent codex`, `cf config`, or `.coding-friend/config.json`
