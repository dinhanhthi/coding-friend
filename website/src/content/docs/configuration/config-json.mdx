...
- Auto-shutdown the daemon after this many milliseconds of inactivity. Set to `0` to disable auto-shutdown. The `cf memory config` wizard prompts in minutes and converts automatically.
- If the daemon stops due to idle timeout, it is **automatically respawned** on the next memory request — no manual restart needed.
