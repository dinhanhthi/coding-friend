...
- **Default**: `"auto"`
- Force a specific search tier instead of auto-detection
- `"auto"` — detect best available (SQLite → MiniSearch → Markdown)
- `"full"` — require SQLite + hybrid search (fails if deps not installed)
- `"lite"` — use MiniSearch daemon only
- `"markdown"` — file-based fallback only
...
- **Type**: `number` (minutes)
- **Default**: `30`
- Auto-shutdown the daemon after this many minutes of inactivity
