...
| `Agent`     | Always safe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `Bash`      | Read-only commands: `ls`, `cat`, `head`, `tail`, `wc`, `file`, `which`, `echo`, `pwd`, `date`, `tree`, `git status`, `git log`, `git diff`, `git branch`, `git show`, `git remote`, `git tag`, `npx tsc --noEmit`, `npx prettier`, `cargo --version`, `cargo tree`, `cargo metadata`, `cargo pkgid`, `cargo locate-project`. Simple commands qualify; pipe chains qualify only if every segment is an allow-prefix (e.g. `ls \| head`, `git log \| grep foo`); standalone `2>&1` stderr redirects are allowed. Chains with `;`, `&&`, `\|\|`, backticks, `$()`, or output redirects are never auto-approved. |
