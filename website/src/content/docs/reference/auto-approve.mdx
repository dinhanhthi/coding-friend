...
<Callout type="warning" title="One key, two hosts">
  `autoApprove: true` enables **both** the Claude Code hook (LLM classifier) and
  the Antigravity hook (deterministic rules). There is no per-host override.
  Codex still uses the separate `autoApproveCodex` key. `autoApproveIgnore` is
  Claude Code only — Antigravity does not read it.
</Callout>
