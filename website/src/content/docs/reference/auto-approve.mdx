...
For Codex CLI, the same `autoApprove: true` key applies. It can also be toggled with a convenience flag:

```bash
cf permission --agent codex --enable-auto-approve
```

This writes `autoApprove: true`. Codex auto-approve is deterministic-only: it does not call the Claude Sonnet classifier and defers unknown actions to Codex native approval.

For Google Antigravity, use the same `autoApprove: true` key. There is no `--enable-auto-approve` flag for `--agent agy`. Antigravity auto-approve is deterministic-only: known-safe reads and working-directory edits can be allowed, destructive actions are denied, and unknown actions return `ask`. The Claude Sonnet classifier is not used from Antigravity hooks.

<Callout type="warning" title="One key, all hosts">
  `autoApprove: true` enables the Claude Code hook (LLM classifier), the
  Antigravity hook, and the Codex hook (both deterministic rules). There is no
  per-host override. `autoApproveIgnore` is Claude Code only — Antigravity and
  Codex do not read it.
</Callout>
...
- Both global and local configs are merged (union), same as
  `autoApproveAllowExtra`.
- **When to use which:**
  - `autoApproveAllowExtra` — you want the hook to silently approve (no prompt, no Claude Code check). Shared across Claude, Antigravity, and Codex.
  - `autoApproveIgnore` — you want Claude Code's native patterns to decide (respects your `.claude/settings.json` allow rules). **Claude Code only** — Antigravity does not read this key.
