...
For Google Antigravity, set `autoApproveAgy: true` in `.coding-friend/config.json` (there is no `--enable-auto-approve` flag for `--agent agy`). Antigravity auto-approve is deterministic-only: known-safe reads and working-directory edits can be allowed, destructive actions are denied, and unknown actions return `ask`. The Claude Sonnet classifier is not used from Antigravity hooks.
