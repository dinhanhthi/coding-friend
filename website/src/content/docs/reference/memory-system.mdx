...
The [MCP](https://modelcontextprotocol.io/) (Model Context Protocol) server bridges Claude Code and memory backends via [stdio transport](https://modelcontextprotocol.io/specification/2025-11-25/basic/transports#stdio) — Claude Code spawns it automatically from the project's `.mcp.json`.

Skills like `/cf-remember` and `/cf-fix` use the MCP tools behind the scenes — you rarely call them directly. Run `cf memory init` or `cf init` to register the server automatically for your project.
