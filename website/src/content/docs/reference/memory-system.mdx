...
- **Auto-reconnect with retry**: if the daemon stops mid-session (e.g., crash), the client automatically respawns it with up to 3 retry attempts before falling back to Tier 3. The daemon runs indefinitely by default (no idle timeout).
