...
### CLAUDE.md sync for conventions

Convention memories (`preference` type) are automatically synced to the project's `CLAUDE.md` file. When you store, update, or delete a convention memory, a corresponding entry is added, updated, or removed under a dedicated `## CF Memory: Conventions` section.

Each entry is a concise one-liner (from the memory's `description` field) tracked via an HTML comment (`<!-- cf:<memory-id> -->`). This ensures Claude Code always has convention rules loaded in its context — without needing to search memory on every session start.
