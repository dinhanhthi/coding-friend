...
Invoked by [`/cf-review [target]`](/docs/skills/cf-review/). The agent **orchestrates** a multi-agent review pipeline — dispatching 5 Claude specialist agents in parallel (plus an optional Codex cross-engine reviewer), then merging results via a reducer agent.

**Review pipeline:**

1. Prepare shared context (diff + changed files + review mode)
2. Dispatch specialist agents in parallel (3 in QUICK mode, 5 in STANDARD/DEEP; Codex opt-in in STANDARD/DEEP)
...
| `cf-reviewer-codex`    | External | Codex cross-engine review (opt-in, STANDARD/DEEP)  |
...
**Codex integration (opt-in):** When `config.codex.enabled = true` and the `codex` CLI is installed, `cf-reviewer-codex` runs as a 6th parallel specialist via the `codex:codex-rescue` subagent. Cross-engine agreement (Claude + Codex flagging the same `file:line`) is treated as the strongest signal and bumps severity one tier. Enable in `.coding-friend/config.json`:
...
Findings from all specialists are collected into 4 severity categories (🚨 Critical, ⚠️ Important, 💡 Suggestions, 📋 Summary). Each finding is tagged with its source layer inline (e.g., `[L3: Security]`, `[L5: Codex]`). Multi-agent agreement on the same issue raises confidence.
...
+ cf-reviewer-codex (opt-in, via codex:codex-rescue)
    ↓ collect outputs
