...
/cf-plan --auto Add a /healthz endpoint
```
...
- **`--auto`** — Orthogonal flag (combines with any mode). After plan approval, runs each phase end-to-end: dispatch tasks → auto `/cf-review` → auto-fix Critical+Important findings (max 2 review rounds) → single conventional commit per phase → continue without prompting. Best when you trust the plan and want a hands-off run. Pair with `--add-tests` to autopilot a TDD-style workflow.
...
Use `/cf-learn` to extract learning notes for yourself — things you discovered during a session that are worth remembering as a developer. For a deeper, conversational breakdown of _what happened and why_ during a task (approach, alternatives, tradeoffs, mistakes, and lessons), use [`/cf-teach`](/docs/skills/cf-teach/) instead.
