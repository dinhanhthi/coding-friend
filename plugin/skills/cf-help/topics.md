...
Paths are relative to `${CLAUDE_PLUGIN_ROOT}` (git checkout: `plugin/`). Repo-only `docs/…` are **not** shipped in the plugin.
...
| `/cf-ask`             | `[question]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `/cf-plan`            | `[task]` · `--fast` (`--quick`) inline unless autopilot · `--hard` extra discovery + rollback · `--auto` autopilot · `--no-auto` force off · `planAuto: true` defaults autopilot on · `--inline` (`--no-file`) chat only, not with autopilot · `--gui` (`--human`) overview · `--model <alias>` pin cf-planner (Claude `opus`/`sonnet`/`haiku`/`fable`; Codex model name; AGY `inherit`/`flash`/`pro`) · `--add-tests` (`--tdd`) to implementers. Resume = `/cf-plan-resume`, not a flag (does not read `planAuto`). · writes `brief.md` in normal/hard                                                                        |
| `/cf-plan-resume`     | `<plan>` path, entry file, or slug. Honors that plan's `auto: true` + `## AUTOPILOT`. Does **not** read `planAuto`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `/cf-plan-review`     | `[plan]` path, entry file, or slug (empty → pick from `docs/plans/`) · `--fix` (`--inline`) apply 🚨/⚠️ immediately · `--codex` / `--gemini` / `--claude` / `--cursor` / `--grok` external reviewers, host flag skipped · writes `review.md`, applies 🚨/⚠️ on confirm unless `--fix`                                                                                                                                                                                                                                                                                                                                          |
| `/cf-later-do`        | `[item]` from `docs/later/` → `/cf-fix` or `/cf-plan`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `/cf-review`          | `[target]` · `--quick` / `--deep` · `--with-codex` (`--codex`) · `--claude` · `--gemini` · `--cursor` · `--grok` · `--out` (no agent flags) · `--fix` apply 🚨/⚠️ + re-review (≤ `review.maxRounds`) · `--commit` (implies `--fix`) commit when clean — only the reviewed uncommitted paths (pathspec), after a secret scan; stops on hook failure · not with a commit range or `--out`. Config: `review.withCodex`, `review.withClaude`, `review.withGrok`, `review.withCursor`, `review.withGemini`, `review.agentTimeout` (300s, enforced on the external subprocess), `review.nativeTimeout` (600s, cooperative budget for an in-session reviewer), `review.maxRounds` (5 — autopilot and `--fix` loop cap). |
...
| `/cf-help`            | `[question]`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `cf-tdd`              | `--add-tests` (`--tdd`) · `--no-tdd` (default) · `--auto`. Config `tdd: true`, `review.maxRounds` (5).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `cf-sys-debug`        | no flags                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `cf-verification`     | no flags                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |

Shared: `--add-tests` / `--tdd` on plan / fix / tdd / implementer.
Shared: `/cf-plan` autopilot = `--auto` **or** config `planAuto: true`; `--no-auto` wins. Resume does not read `planAuto`.
...
| `cf init`                                                    | Per-project `docs/` + `.coding-friend/config.json` (includes TDD + Plan autopilot / `planAuto`) |
| `cf config`                                                  | Interactive config (TDD, Plan autopilot / `planAuto`, review, memory, …)                        |
...
| `planAuto`              | `false`                                     | Default `/cf-plan` autopilot without `--auto`. Set via `cf config` / `cf init` (Plan autopilot, next to TDD). Local overrides global. `--no-auto` forces off for one run. `/cf-plan-resume` ignores it (frontmatter `auto: true` only). |
...
| `review.withClaude`     | `false`                                     | Always add Claude on `/cf-review`                                                                                                                                                                                                       |
| `review.withGrok`       | `false`                                     | Always add Grok on `/cf-review`                                                                                                                                                                                                         |
| `review.withCursor`     | `false`                                     | Always add Cursor on `/cf-review`                                                                                                                                                                                                       |
| `review.withGemini`     | `false`                                     | Always add Gemini on `/cf-review`                                                                                                                                                                                                       |
...
| `review.maxRounds`      | `5`                                         | `/cf-review` fix-loop cap (`/cf-plan --auto`, `/cf-tdd --auto`, `/cf-review --fix`)                                                                                                                                                     |
...
Markdown in `{docsDir}/memory/` is the source of truth: `fact`→`features/`, `preference`→`conventions/` (syncs to CLAUDE.md), `context`→`decisions/`, `episode`→`bugs/`, `procedure`→`infrastructure/`.
...
- **Decide then build:** `/cf-advise` → `/cf-plan` → `/cf-plan-review` → implement (`cf-tdd`) → `/cf-review` → `/cf-commit` → `/cf-ship`
