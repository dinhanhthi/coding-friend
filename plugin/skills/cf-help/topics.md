...
| `/cf-review`          | `[target]` · `--quick` / `--deep` · `--with-codex` (`--codex`) · `--claude` · `--gemini` · `--cursor` · `--grok` · `--out` (no agent flags) · `--fix` apply 🚨/⚠️ + re-review (≤ `review.maxRounds`) · `--commit` (implies `--fix`) commit when clean — only the reviewed uncommitted paths (pathspec), after a secret scan; stops on hook failure · not with a commit range or `--out`. Config: `review.withCodex`, `review.agentTimeout` (300s, enforced on the external subprocess), `review.nativeTimeout` (600s, cooperative budget for an in-session reviewer), `review.maxRounds` (5 — autopilot and `--fix` loop cap). |
