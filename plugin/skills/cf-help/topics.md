...
Paths below are relative to the plugin root (`${CLAUDE_PLUGIN_ROOT}`). When working in the coding-friend git checkout, the same files live under `plugin/`. Repo-only docs (`docs/…` at the repo root) are listed separately — they are **not** shipped inside the installed plugin.
...
**Invocation:** slash `/cf-*` (Claude / omp / agy) or `$cf-*` (Codex). Auto-invoked skills load from their `description`. Agents run in a forked session. Distinguish *doing* a skill now from *talking about* a skill — `/cf-help` is the talk-about path.
...
Resolve plugin files as `${CLAUDE_PLUGIN_ROOT}/<path>`. If that miss (developing this repo), try `plugin/<path>` from the workspace root.
...
| `preference` | `conventions/` | Conventions (also sync to CLAUDE.md) |
