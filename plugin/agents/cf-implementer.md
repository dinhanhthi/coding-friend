...
- Read `${CLAUDE_PLUGIN_ROOT}/lib/protocols/test-scope.md` first — skip the test when it adds no protection, and say why in the Report
...
- In TDD mode: NEVER write production code before a failing test, unless `${CLAUDE_PLUGIN_ROOT}/lib/protocols/test-scope.md` says no test is needed (say why in the Report)
- ONE behavior per test, and only tests `${CLAUDE_PLUGIN_ROOT}/lib/protocols/test-scope.md` allows (TDD mode)
...
This writes `<docsDir>/later/YYYY-MM-DD-<name>.md` with frontmatter (slug, problem, conversation_id). Trivial inline fixes that the current task clearly implies stay inline. This is an in-repo audit trail, independent of the `spawn_task` tool. Do NOT modify any plan file.
