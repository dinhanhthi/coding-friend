...
2. **Skills inheritance path (R2, chưa chốt live)** — omp inherit `~/.claude` priority 80. Chưa verify live: đọc `~/.claude/skills/` trực tiếp, hay cache `~/.claude/plugins/cache/*/skills/`? User **không** cài Claude Code có thể **không** thấy skill `cf-*`. Fallback (chưa ship): copy vào `~/.omp/skills/` lúc install.
