...
| `PreToolUse` matcher `*`                                                                                                                            | [`auto-approve.agy.cjs`](../plugin-antigravity/hooks/auto-approve.agy.cjs)                                                                            | PreToolUse (auto-approve) | Opt-in `autoApproveAgy: true`. Default / thiếu key → `{decision: "ask"}`. Deterministic, không LLM.    |
...
cf init --agent agy                # 5. mỗi project: docs/ + AGENTS.md
...
5. **Privacy/scout fail open** — JSON stdin hỏng → allow. omp **fail
   ...
   chạy).
6. **`autoApproveAgy` opt-in** — default `ask`. `cf permission --agent agy`
   là no-op: agy tự quản `/permissions`; auto-approve CF chỉ là config key.
...
| Auto-approve không allow               | `autoApproveAgy` default off                                                | Set `true` trong `.coding-friend/config.json`                              |
