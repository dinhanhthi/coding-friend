...
- **Dispatch `<agent>`** — run the Coding Friend subagent with that name: spawn the `<agent>` custom agent. Several at once → dispatch all in one message, then wait for every result. Pass a `model` only when the skill says so.
