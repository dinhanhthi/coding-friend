...
Dispatch the **cf-implementer** agent. Spawn the `cf-implementer` custom agent. Pass the Step 3b context file.
...
2. Delegate to **cf-writer** by spawning the `cf-writer` custom agent. Absolute `file_path`: `MAIN_REPO_ROOT` from bootstrap (fallback `pwd`), config from `CF_CONFIG_FILE`, docs base `CF_DOCS_ROOT`.
...
Automatically invoke `$cf-review` — load `$cf-review`. Do not ask first.
...
2. If the user agrees, invoke `$cf-learn` — load `$cf-learn`
