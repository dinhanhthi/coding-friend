...
> Requirements:
>
> 1. `--add-tests` and no regression test → first write a test that fails on the pre-fix code for the intended reason (see `${PLUGIN_ROOT}/lib/protocols/test-scope.md`). Else fix directly.
