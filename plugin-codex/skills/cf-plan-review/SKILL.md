...
mkdir -p "${CF_DOCS_ROOT}/reviews" && \
  bash "${PLUGIN_ROOT}/skills/cf-plan-review/scripts/build-plan-review-prompt.sh" \
    <entry-file> <docsDir> \
...
Use the Codex subagent workflow with a fresh subagent. Do **not** set `context: fork`. Do **not** pass a `subagent_type`. A fresh default-type subagent inherits this skill's model.
...
**When any external source survived:** merge via **cf-reviewer-reducer** (spawn the `cf-reviewer-reducer` custom agent). Source 1 is the in-session report; each surviving external review is a numbered source:
