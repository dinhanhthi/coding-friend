...
2. Anything more specific → Read `${PLUGIN_ROOT}/skills/cf-help/topics.md` (or `plugin/skills/cf-help/topics.md` in this repo) — topic → source map, flags, hooks, CLI, config, native prompt reduction.
3. Then Read the source that index points to (`skills/<name>/SKILL.md`, `agents/<name>.md`, `hooks/<file>`).
4. If still unclear: Glob / Grep under `${PLUGIN_ROOT}/skills/`, `agents/`, `hooks/`.

### Step 2: Provide overview (if general question)

Coding Friend is a lean toolkit for disciplined engineering workflows in Codex CLI. Check skills first; TDD optional (`--add-tests` or `tdd: true`); verify before claiming done.
...
- `$cf-plan [task]` — ⚡⚡ — Phased plans. `--fast`/`--quick`, `--hard`, `--auto`, `--inline`/`--no-file`, `--gui`/`--human`, `--model <name>` pin the model for cf-planner at the brainstorm step (Codex model name, e.g. `gpt-5.5`; Claude aliases are not valid). Autopilot review-fix cap: `review.maxRounds` (default 5). Normal/hard mode also writes `brief.md` (request, Q&A, assumptions) for `$cf-plan-review`.
...
- **cf-reviewer** — ⚡ — Reviews the diff itself, 5 layers. `$cf-review` dispatches it directly: 1 in QUICK, 1 in STANDARD, 2 in DEEP — the second is **cf-reviewer-security** (medium reasoning effort). Flat graph — no sub-dispatch, no merge agent.
- **cf-reviewer-plan** (medium reasoning effort), **cf-reviewer-quality** (low reasoning effort), **cf-reviewer-tests** (low reasoning effort), **cf-reviewer-rules** (low reasoning effort), **cf-reviewer-reducer** (low reasoning effort) — ⚡ — retained, directly callable; off `$cf-review`'s default path
