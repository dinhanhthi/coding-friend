...
- `$cf-plan [task]` — ⚡⚡ — Phased plans. `--fast`/`--quick`, `--hard`, `--auto`, `--no-auto`, `--inline`/`--no-file`, `--gui`/`--human`, `--model <name>` pin the model for cf-planner at the brainstorm step (Codex model name, e.g. `gpt-5.5`; Claude aliases are not valid). Autopilot also from config `planAuto: true` (local overrides global) unless `--no-auto`. Autopilot review-fix cap: `review.maxRounds` (default 5). Normal/hard mode also writes `brief.md` (request, Q&A, assumptions) for `$cf-plan-review`.
