...
- `$cf-plan [task]` — ⚡⚡ — Phased plans. `--fast`/`--quick`, `--hard`, `--auto`, `--inline`/`--no-file`, `--gui`/`--human`, `--model <name>` pin the model for cf-planner at the brainstorm step (Codex model name, e.g. `gpt-5.5`; Claude aliases are not valid).
