...
```bash
bash "${PLUGIN_ROOT}/skills/cf-review/scripts/run-codex-review.sh" "${CF_DOCS_ROOT}/reviews/<label>-result-codex.md"
```
...
- **DEEP mode**: Launch the **cf-explorer agent** to understand callers, dependencies, and data flows around the changed files. Use the **Agent tool** with `$cf-explorer`. Pass:
...
Use the **Agent tool** with `$cf-reviewer`. Pass the full context:
...
3. Normalize the raw Codex result into the standard 4-section format:
   ...
   ```bash
   bash "${PLUGIN_ROOT}/skills/cf-review/scripts/normalize-codex-review.sh" "${CF_DOCS_ROOT}/reviews/<label>-result-codex.md"
   ```
...
**When `codex=true`:** merge the two reviews through the reducer so findings are deduplicated and severity-ranked across both sources. Dispatch the **cf-reviewer-reducer** agent (Agent tool, `$cf-reviewer-reducer`) with both inputs concatenated:
