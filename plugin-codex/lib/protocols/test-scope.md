...
- Not every change needs a new test: renames, copy, config, docs, pure refactors, and small reversible implementation tweaks usually need none. Cover only the paths the change puts at risk, not every failure or edge case. Security-sensitive changes (auth, input validation, path/command handling, crypto) are never exempt.
