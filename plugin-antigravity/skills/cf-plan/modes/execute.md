...
> **No-file mode guard** — if no plan file was written (cf-plan `--inline`, or single-phase `--fast` without `--auto`), there is no `README.md`/phase file to edit: replace every "Edit the plan file / Progress table" checkpoint below with a checklist update on the matching task. `/cf-plan-resume` always has a plan file, so this never applies when resuming.
...
Dispatch **cf-implementer** (`invoke_subagent` with agent `cf-implementer`) per task:
...
2. Call `invoke_subagent` with agent `cf-implementer` once per task in parallel, wait for all agents, and collect each result.
