...
>
> **Diff:** read `/tmp/coding-friend/review/<run-id>/diff.txt` (the Step 2 snapshot — every hunk exactly once, read-only). Missing → re-run `bash "<plugin-root>/skills/cf-review/scripts/gather-diff.sh"` with the same flags.
>
