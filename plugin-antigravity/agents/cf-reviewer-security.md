...
description: >
  Security review specialist. Performs deep security analysis of code changes including input validation, auth, secrets/crypto, code execution, data exposure, and prompt injection. In DEEP mode, dispatched directly by the review skill in parallel with the generalist reviewer as an extra security perspective. Includes exploit scenarios for Critical findings. Traces data flow from untrusted input through processing to sensitive operations.
model: pro
---
